...
function StackVinyl({ track, stackIndex, isActive, onSelect }: StackVinylProps) {
  ...
  const z = stackIndex % 2 === 0 ? 0 : 0.16
  const yRot = -0.1 + stackIndex * 0.07

  return (
    <group position={[x, TABLE_SURFACE_Y, z]} rotation={[0, yRot, 0]}>
      <mesh
        castShadow
        receiveShadow
        onClick={(event) => {
      ...
      >
        <boxGeometry args={[0.98, 0.018, 1.06]} />
      ...
      <mesh position={[0, 0.011, -0.04]} rotation={[-Math.PI / 2, 0, 0]}>
